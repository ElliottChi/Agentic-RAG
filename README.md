...
## 專案簡介 (Introduction)
本專案是一個基於 **Agentic Workflow** 的企業級知識問答系統原型。
...
系統落地於 **LINE Bot** 平台，並解決了 **Cold Start** 與 **Stateless** 等問題，實現了高精準度、低延遲的法規諮詢服務。
...
### 1. Agentic Workflow
系統不再被動檢索，而是具備「大腦」。
- **Router**：自動判斷使用者意圖（閒聊 vs. 查法規 vs. 計算罰款）。
...
- **Stage 1 (Recall)**：使用 **Hybrid Search**，結合 **BM25 (關鍵字)** 與 **ChromaDB**，確保檢索廣度。
- **Stage 2 (Precision)**：引入 **Cross-Encoder Reranker** 模型，對檢索結果進行深度語意評分，大幅降低幻覺。
